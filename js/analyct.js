 // --- MODIFIED SCRIPT TO APPLY SETTINGS (LIKE index.html) ---
    (async () => {
        try {
            ...
            // Use a cache-busting query parameter to get the latest settings
            ...
            if (settings.logo_url) {
                // Find all elements with the class 'logo' and update them
                const logoElements = document.querySelectorAll('.logo'); 
                logoElements.forEach(el => {
                    // Replace the text content with an image tag
                    el.innerHTML = `<img src="${settings.logo_url}" alt="Site Logo">`;
                });
            }
